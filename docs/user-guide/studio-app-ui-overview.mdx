...
- **Select and compare**: A checkbox on each row, plus one in the header to select the whole page. Selecting runs raises a bar at the bottom of the list with a **Compare** button, enabled once exactly two runs are selected — see [Compare Runs](/docs/user-guide/compare-runs).
...
- **Notices:** A megaphone button opening the notice inbox, badged with the number of unread notices. Shown on every page. Notices are published by whoever operates the deployment — see [Host Page Integration](/docs/reference/host-integration#reading-notices).
...
Some preferences only make sense when the feature they configure is switched on — those appear here once it is, and are hidden otherwise.

### Beta Features

Enable or disable features that are still under active development. Beta features are toggled per-browser and do not affect other users.

Not every entry here is off by default. A beta feature that has been released to everyone still appears in this list, switched on, so you can opt out of it if you would rather not have it. [Compare runs](/docs/user-guide/compare-runs) and [conditional execution](/docs/user-guide/pipeline-editor#conditional-execution) are both on by default in this way.
